...
bool AudioFile<T>::decodeWaveFile(std::vector<uint8_t>& fileData) {
    ...
    bool isFloat = (audioFormat == 3);  // IEEE float

    // check that the audio format is PCM or IEEE float
    if (audioFormat != 1 && audioFormat != 3) {
        std::cout << "ERROR: this is a compressed .WAV file and this library "
                     "does not support decoding them at present"
                  << std::endl;
        return false;
    }
    ...
    if (numChannels < 1) {
        std::cout << "ERROR: this WAV file has no channels"
                  << std::endl;
        return false;
    }
    ...
    if (isFloat) {
        if (bitDepth != 32) {
            std::cout << "ERROR: float WAV must be 32-bit" << std::endl;
            return false;
        }
    } else {
        if (bitDepth != 8 && bitDepth != 16 && bitDepth != 24) {
            std::cout
                << "ERROR: this file has a bit depth that is not 8, 16 or 24 bits"
                << std::endl;
            return false;
        }
    }
    ...
    for (int i = 0; i < numSamples; i++) {
        for (int channel = 0; channel < numChannels; channel++) {
            ...
            if (isFloat && bitDepth == 32) {
                ...
                float sampleAsFloat;
                uint32_t asInt = (uint32_t)fileData[sampleIndex] |
                                 ((uint32_t)fileData[sampleIndex + 1] << 8) |
                                 ((uint32_t)fileData[sampleIndex + 2] << 16) |
                                 ((uint32_t)fileData[sampleIndex + 3] << 24);
                std::memcpy(&sampleAsFloat, &asInt, sizeof(float));
                samples[channel].push_back((T)sampleAsFloat);
            } else if (bitDepth == 8) {
...
bool AudioFile<T>::decodeAiffFile(std::vector<uint8_t>& fileData) {
    ...
    if (indexOfSoundDataChunk == -1 || indexOfCommChunk == -1 ||
        headerChunkID != "FORM" || format != "AIFF") {
    ...
    if (numChannels < 1 || numChannels > 2) {
        std::cout << "ERROR: this AIFF file seems to be neither mono nor "
                     "stereo (perhaps multi-track, or corrupted?)"
                  << std::endl;
        return false;
    }
    ...
    for (int i = 0; i < getNumSamplesPerChannel(); i++) {
        for (int channel = 0; channel < getNumChannels(); channel++) {
            if (bitDepth == 8) {
                ...
                int16_t sampleAsInt =
                    (int16_t)(samples[channel][i] * (T)32768.);
...
void AudioFile<T>::addStringToFileData(std::vector<uint8_t>& fileData,
                                       std::string s) {
    for (int i = 0; i < s.length(); i++) fileData.push_back((uint8_t)s[i]);
}
